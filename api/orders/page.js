...
//   let rows = await getOrders({ shopDomain: shop || undefined });
...
export default async function handler(req, res) {
  ...
  const limitParam = req.query.limit ? Number(req.query.limit) : 0;
  const hardLimit = limitParam > 0 ? Math.min(limitParam, 1000) : null;

  // already returns newest first (created_at desc) in db.js
  const rows = await getOrders({ shopDomain: shop || undefined });
  const total = rows.length;

  // no pagination if no limit
  if (!hardLimit) {
    return res.status(200).json({ ok: true, items: rows, nextCursor: null, total });
  }
