...
// import { upsertOrder, writeLineItems, logWebhook } from "../lib/db.js";
...
export default async function handler(req, res) {
  ...
  const allowBypass =
    ALLOW_DEBUG_BYPASS.toLowerCase() === "true" &&
    DEBUG_BYPASS_TOKEN &&
    req.headers["x-debug-bypass"] === DEBUG_BYPASS_TOKEN;
  ...
  try {
    await upsertOrder(order); // writes to Neon
    action = "upserted";

    const batchTs = Date.now();
    if (Array.isArray(items) && items.length) {
      await writeLineItems(items, batchTs); // writes to Neon
    }
  } catch (e) {
    errMsg = e?.message || String(e);
  }
